...
def analyze_trends(df_list, metadata_list, run_names, distance=20, prominence=1.0, width=1):
    peaks_troughs_data = []
    distances_data = []
    for df, metadata, run_name in zip(df_list, metadata_list, run_names):
        peaks, troughs = find_peaks_troughs(df['dBm'], distance=distance, prominence=prominence, width=width)
...
if uploaded_files:
    ...
    distance = st.sidebar.slider("Distance between peaks", 1, 100, 20)
    prominence = st.sidebar.slider("Prominence of peaks", 0.1, 10.0, 1.0)
    width = st.sidebar.slider("Width of peaks", 1, 20, 1)
    ...
    for df, name, metadata in zip(filtered_data_frames, filtered_run_names, filtered_metadata_list):
        ...
        dbm = df['dBm']
        peaks, troughs = find_peaks_troughs(dbm, distance=distance, prominence=prominence, width=width)
    ...
    if len(filtered_data_frames) >= 2:
        ...
        for i in range(len(filtered_data_frames)):
            for j in range(i + 1, len(filtered_data_frames)):
                ...
                peaks1, troughs1 = find_peaks_troughs(df1['dBm'], distance=distance, prominence=prominence, width=width)
                peaks2, troughs2 = find_peaks_troughs(df2['dBm'], distance=distance, prominence=prominence, width=width)
    ...
    trends_df, distances_df = analyze_trends(filtered_data_frames, filtered_metadata_list, filtered_run_names, distance=distance, prominence=prominence, width=width)
    st.dataframe(trends_df)  # Making the table scrollable
    # Filter data by selected step size
    filtered_distances_df = distances_df[distances_df['step'].isin(selected_step_size)]

    # Plot distances between consecutive peaks and troughs
    ...
    # Box plot of distances between consecutive peaks and troughs for each index
    ...
    # Plot the average of all plots
    st.subheader("Average Plot of dBm vs Position")
    # Compute average dBm values for each position
    ...
    # Apply peak and trough detection on the averaged data
    ...
    # Extract peak and trough positions
    ...
    # Create the average plot with peaks and troughs
    fig_avg = go.Figure()

    # Plot the average dBm vs Position line
    ...
    # Adjust lengths of peak_positions and trough_positions to be equal
    ...
    # Display the peak and trough positions data
    ...
    # Create plot for consecutive peak and trough distances
    ...
    # Create box plot for consecutive peak and trough distances
    fig_box_avg = go.Figure()

    # Box plot for peak distances
...
if st.button('Run Machine Learning Analysis'):
    ...
    if dl_model_file:
        best_dl_model = load_model(dl_model_file)
    else:
        try:
            # Check for NaN or infinite values
            if X_train.isnull().values.any() or np.isinf(X_train.to_numpy()).any() or y_train.isnull().values.any() or np.isinf(y_train.to_numpy()).any():
                st.write("Warning: Input data contains NaN or infinite values. Please clean your data.")
            else:
                tuner = RandomSearch(
                    build_model,
                    objective='val_mse',
                    max_trials=10,  # Increase number of trials
                    executions_per_trial=3,  # Increase executions per trial for better averaging
                    directory='tuner_dir',
                    project_name='motor_sweep'
                )

                tuner.search(X_train_scaled, y_train, epochs=100, validation_split=0.2, verbose=2)  # Increase number of epochs

                best_hps = tuner.get_best_hyperparameters(num_trials=1)[0]
                st.write(f"Best hyperparameters found: {best_hps.values}")

                best_dl_model = tuner.hypermodel.build(best_hps)
                ...
                st.write(f"Deep Learning model saved at {os.path.abspath('best_dl_model.keras')}")
        
        except Exception as e:
            st.write(f"An error occurred during model training: {str(e)}")
    ...
    # If deep learning model exists, create and display its plot
    if best_dl_model:
        # Deep Learning predictions
        new_features_scaled = scaler.transform(new_features)
        predicted_dbm_dl = best_dl_model.predict(new_features_scaled).flatten()
        create_and_display_plot(positions, predicted_dbm_dl, "Deep Learning")
    else:
        st.write("Deep Learning model not available. Skipping Deep Learning plot.")
